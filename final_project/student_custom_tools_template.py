...
def select_feasible_itinerary(
    ...
    *,
    ...
    flights = session.dispatch("search_flights", {"origin": episode["origin"], "destination": city, "max_results": 20}).get("items", [])
    hotels = session.dispatch("search_hotels", {"city": city, "max_results": 20}).get("items", [])
    rests = session.dispatch("search_restaurants", {"city": city, "max_results": 20}).get("items", [])
    acts = session.dispatch("search_activities", {"city": city, "max_results": 20}).get("items", [])
    ...
    #try:
        #result = _try_bundle_upgrade(session, episode, result, nights, budget)
    #except Exception:
...
def _try_bundle_upgrade(
    ...
    No LLM calls — uses session.dispatch("get_partner_promotions") directly.
    """
    city = episode["city"]
    family = episode.get("family", "")
    mz = episode.get("meeting_zone")
    ...
    promos = session.dispatch("get_partner_promotions", {"city": city, "family": family, "max_results": 20}).get("items", [])
    if not promos:
        return current
